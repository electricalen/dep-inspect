...
/**
 * `dep-inspect scan` — scan project dependencies.
 * GitHub data is OFF by default (use --github to enable).
 */
export async function scanCommand(opts: ScanOptions): Promise<void> {
  ...
  try {
    ...
    if (opts.json) {
      ...
      console.log(formatScan(analysis.value, policy, { details: opts.details }))
    }
  } finally {
    cleanup()
  }
